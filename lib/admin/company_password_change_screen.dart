...
class CompanyPasswordChangeScreen extends StatefulWidget {
  ...
  @override
  State<CompanyPasswordChangeScreen> createState() =>
      _CompanyPasswordChangeScreenState();
}

class _CompanyPasswordChangeScreenState
    extends State<CompanyPasswordChangeScreen> {
  ...
  Future<void> sendOtp() async {
    ...
    final response = await http.post(
      Uri.parse('$baseURL/company/check-email'),
    ...
    if (response.statusCode == 200) {
      ...
      if (data['exists']) {
        ...
        await http.post(
          Uri.parse('$baseURL/company/store-otp'),
          headers: {'Content-Type': 'application/json'},
          body: jsonEncode({'email': email, 'otp': otp, 'type': 'company'}),
        );
  ...
  Future<void> verifyOtpAndShowPasswordFields() async {
    ...
    final response = await http.post(
      Uri.parse('$baseURL/company/verify-otp'),
      headers: {'Content-Type': 'application/json'},
      body: jsonEncode({'email': email, 'otp': otp, 'type': 'company'}),
    );
  ...
  Future<void> changePassword() async {
    ...
    final response = await http.post(
      Uri.parse('$baseURL/company/change-password'),
      headers: {'Content-Type': 'application/json'},
